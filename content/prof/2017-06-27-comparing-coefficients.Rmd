...
比如，检验线性回归是不是有内生性：H0：不具有内生性 H1：具有内生性那么`b1`就是 ols 回归结果，`b2`就是 iv 的回归结果。同样，检验固定效应还是随机效应，`b1`是随机效应结果，`b2`是固定效应结果。但如果比较的是不是组回归的系数，显然不满足 Hausman test 的前提，所以绝对不能用 Hausman test。
